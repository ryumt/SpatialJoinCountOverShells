...
produceBacklogIndex(void *arg)
{
	...
	/* all files' index have been created */
	if (*bs->file_index_ptr >= bs->nparticle_files)
		return ;
	
	nbacklogs = fetch_and_add(bs->nbacklogs_ptr, 1);
...
static inline 
void 
noBacklogExecution(BacklogStuff *bs)
{
	SameThreadPool *stp;
	JoinContext jc;
	int i;
	...
	stp = constructSameThreadPool(bs->nthreads);
	for (i = 0; i < bs->nparticle_files; i++) {
		memcpy(&jc, bs->jc_template, sizeof(jc));
		stp->nworkers = bs->nsetups;
		/* construct index */
		jc.idx = constructIndex(1, bs->particle_files[i], bs->particle_compressed);
		stp->nworkers = bs->nthreads;
...
static inline 
void 
withBacklogExecution(BacklogStuff *bs)
{
	/* thread pool: processing index construction & search */
	ThreadPool *tp = NULL;
	/* queue that holds backlog JoinContext(search request) */
	CasQueue *backlog_queue = NULL;
	int i, j, k;
	
	
	puts("  -- with backlog execution");
	/* construct queue that holds backlog indexes */
	backlog_queue = constructCasQueue();
	/* construct worker thread pool */
	tp = constructThreadPool(bs->nthreads);
	/* setup arguments for worker thread */
	bs->tp = tp;
	bs->backlog_queue = backlog_queue;
	
	for (j = 0; j < bs->first_nbacklogs; j++)
	...
	for (i = 0; i < bs->nparticle_files; i++) {
		...
		for (j = *bs->nbacklogs_ptr; j < bs->maxbacklogs; j++) {
		...
		MEASURE_TIME_START();
		while (deqCasQueue(backlog_queue, (void **)&jc) == false)
			;
		fetch_and_add(bs->nbacklogs_ptr, -1);
	      ...
	      int nthreads, int nsetups, JoinContext *jc_template)
{
	...
	int file_index = 0;
	BacklogStuff bs = {
		NULL, /* ThreadPool pointer: will be set later */
		NULL, /* CasQueue pointer : will be set later */
		&nbacklogs, maxbacklogs, first_nbacklogs, first_nsetups, 
		nthreads, nsetups, 
		jc_template, 
		particle_files, &file_index, 
		nparticle_files, particle_compressed, 
		NULL /* Pseudo-SameThreadPool pointer: will be set in withBacklogExecution() */
	};
	
	if (maxbacklogs == 0)
		noBacklogExecution(&bs);
	else 
		withBacklogExecution(&bs);
}
...
threadJoin(void *arg)
{
	...
	while (jc->jtc.nprows < jc->bh->nhalos) {
		...
		for (row = beg; row < end; row++) {
			...
			multiRangeCountWithPriodicBoundRTree(jc->idx, &mrcc);
		} 	
	}
